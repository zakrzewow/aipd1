...
def main():
    ...
    ## sidebar
    selected_tab, file_name, app, frequencyApp = tabs.sidebar.run()
    ...
    if selected_tab == "Przebieg czasowy pliku audio":
        ...
        tabs.project_1.przebieg_czasowy.run(app)
    ...
    if selected_tab == "Określanie fragmentów muzyka vs. \n mowa":
        ...
        if len(lster_list) == 1:
            ...
            fig = app.plot_music_vs_talks(
                lster_list, lster_threshord, "Muzyka vs. mowa"
            )
            st.plotly_chart(fig)
    ...
    if selected_tab == "Częstotliwość kratniowa":
        ...
        selected_window_function = st.selectbox(
            "Wybierz funkcję okienkową", window_options
        )
