...
class TimeApp(App):
    ...
    def SR(frame: Frame, vol_min_value=0.0, vol_max_value=0.05, zcr_min_value=0.5, zcr_max_value=2):
        volume = TimeApp.volume(frame)
        zcr = TimeApp.ZCR(frame)
    ...
    def LSTER(frames: List[Frame], window_duration: float = 1) -> float:
        ...
        ste_values = [TimeApp.STE(frame) for frame in frames]
        low_ste_count = 0
    ...
    def proper_LSTER(self):
        ...
        for index, chunk in enumerate(chunks):
            ...
            if len(frames) > 0:
                lster_value = TimeApp.LSTER(frames)
    ...
    @staticmethod
    def VSTD(frames):
        Volumes = [TimeApp.volume(item) for item in frames]
    ...
    @staticmethod
    def ZSTD(frames):
        zcr = [TimeApp.ZCR(item) for item in frames]
        return np.std(zcr)

    @staticmethod
    def HZCRR(frames):
        N = len(frames)
        zcr_values = [TimeApp.ZCR(frame) for frame in frames]
    ...
    def spectral_centroid_std(frames: List[Frame]) -> float:
        ...
        spectral_centroids = [TimeApp.spectral_centroid(frame) for frame in frames]
    ...
    def spectral_bandwidth(frame: Frame) -> float:
        ...
        # Calculate the spectral centroid
        spectral_centroid = TimeApp.spectral_centroid(frame)
    ...
    def spectral_bandwidth_std(frames: List[Frame]) -> float:
        ...
        spectral_bandwidths = [TimeApp.spectral_bandwidth(frame) for frame in frames]
